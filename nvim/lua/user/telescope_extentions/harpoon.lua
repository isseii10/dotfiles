-- https://github.com/ThePrimeagen/harpoon/pull/512/files#diff-138b3f983b94f299ceb42b71bcbe06a83922e9a9830c2891bfff909fe6cfb243
...
if not has_harpoon then
  error "This extension requires harpoon.nvim"
end
...
harpoon_finders.file_mark_finder = function(opts)
...
local harpoon_sorters = {}

harpoon_sorters.file_mark_sorter = function(opts)
...
local harpoon_actions = {}

harpoon_actions.delete_mark_selections = function(prompt_bufnr)
  local selections = {}
...
harpoon_actions.delete_mark_selections_prompt = function(prompt_bufnr)
  vim.ui.input({
    ...
    if input_str == "y" or input_str == "yes" then
      local deletion_count = harpoon_actions.delete_mark_selections(prompt_bufnr)
...
harpoon_actions.move_mark_next = function(prompt_bufnr)
  -- get current index
...
harpoon_actions.move_mark_previous = function(prompt_bufnr)
  -- get current index
  local current_selection = action_state.get_selected_entry()
  local current_index = current_selection.index
...
local marks = function(opts)
  ...
  pickers
      .new(opts, {
        ...
        finder = harpoon_finders.file_mark_finder(opts),
        sorter = harpoon_sorters.file_mark_sorter(opts),
        previewer = conf.file_previewer(opts),
        attach_mappings = function(_, map)
          map("i", "<c-d>", harpoon_actions.delete_mark_selections_prompt)
          map("n", "<c-d>", harpoon_actions.delete_mark_selections_prompt)
          map("n", "dd", harpoon_actions.delete_mark_selections_prompt)
          map("i", "<c-p>", harpoon_actions.move_mark_previous)
          map("n", "<c-p>", harpoon_actions.move_mark_previous)
          map("i", "<c-n>", harpoon_actions.move_mark_next)
          map("n", "<c-n>", harpoon_actions.move_mark_next)
          return true
        end,
      })
      :find()
